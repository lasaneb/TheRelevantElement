...
def load_contract():
    ...
    with open(Path('./contracts/compiled/CreateArtToken.json')) as f:
        artwork_abi = json.load(f)

    contract_address = os.getenv("ARTWORK_SMART_CONTRACT_ADDRESS")
...
if st.button("Register Artwork"):
    ...
    tx_hash = contract.functions.registerArtwork(
        address,
        artwork_uri
    ).transact({'from': os.getend("owner_address"), 'gas': 1000000})
